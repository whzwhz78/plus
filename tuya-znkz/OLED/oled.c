...
void OLED_ShowIntegerNumber(u8 x,u8 y,int num,u8 Char_Size)  //ÏÔÊ¾ÕûÊý£¬°üº¬Õý¸º
{
	unsigned char len=0;
	int buf=num;
	while(buf)
...
/*
ÃèÊö£ºÏÔÊ¾ÕûÊý(¿ÉÏÔÊ¾Õý¸ºÊý)£¨-65535~65535£©
ÊäÈë£º
	x:ÏÔÊ¾µÄxÆðÊ¼Î»ÖÃ
	y:ÏÔÊ¾µÄyÆðÊ¼Î»ÖÃ
	num:ÒªÏÔÊ¾µÄÊý×Ö
	Char_Size:ÒªÏÔÊ¾Êý×ÖµÄ´óÐ¡
¾ÖÏÞ£ºÄ¿Ç°Ö»ÄÜÏÔÊ¾È·¶¨·¶Î§µÄÐ¡Êý£¬ÇÒÐ¡Êý²¿·ÖÏÔÊ¾²»×¼È·
*/
void OLED_ShowFloatNumber(u8 x,u8 y,float num,u8 Char_Size)
{
	...
	zheng=(int)num;
	xiao=((num-zheng)*100)/1;	//ÏÔÊ¾Ð¡ÊýµãºóÁ½Î»
	while(zheng)	//¼ÆËãÕûÊý²¿·ÖÎ»Êý
	{
		zheng_len++;
		zheng/=10;
	}
	xiao_len=2;
	zheng=(int)num;	//ÔÚ¼ÆËãÕûÊý²¿·ÖÊýÖµÊ±£¬Ô­¸³Öµ±»´Û¸Ä£¬ÕâÀïÖØÐÂ¸³Öµ
	if(polarity_flag)
	{	
		if(Char_Size==16)
		{
